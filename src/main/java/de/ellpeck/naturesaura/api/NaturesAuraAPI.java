...
public final class NaturesAuraAPI{
    ...
    public static final Map<String, Class<? extends IAuraSupplier>> SUPPLIER_REGISTRY = new HashMap<String, Class<? extends IAuraSupplier>>();
    ...
    public static IAuraHandler getAuraHandler(World world){
        return apiHandler.getAuraHandler(world);
    }

    public static void registerAuraType(AuraType type){
        AURA_REGISTRY.put(type.getRegisteredName(), type);
    }
    ...
    public static void registerAuraSupplier(String name, Class<? extends IAuraSupplier> suppClass){
        SUPPLIER_REGISTRY.put(name, suppClass);
    }
}
